		...
		if "link" in i:
			for j in i["link"]:
				if "host" not in j['link-id']:
					print("[DEBUG] Traitement du lien :", j["link-id"])
					src = j["link-id"].split(":")
					print("[DEBUG] src =", src)
					srcPort = src[2]
					...
					dstToSrc = dst[1] + "::" + src[1]
					linkPorts[dstToSrc] = dstPort + "::" + srcPort
